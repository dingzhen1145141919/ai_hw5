...
# ====================== 1. 配置（核心改动：输出目录改为output_blip_3，文本用pro版本） ======================
...
# 核心改动1：输出目录改为 models/blip_fusion/output_blip_3
OUTPUT_DIR = os.path.join(ROOT_DIR, "models/blip_fusion/output_blip_3")
os.makedirs(OUTPUT_DIR, exist_ok=True)  # 确保目录存在

# Training Parameters（完全不变）
...
class BLIPEarlyFusionDataset(Dataset):
    ...
    def _filter_valid_files(self, label_df):
        ...
        for _, row in label_df.iterrows():
            ...
            # 核心改动2：文本文件改为 {guid}_pro.txt（你可根据实际文件名调整）
            txt_path = os.path.join(DATA_DIR, f"{guid}_pro.txt")
    ...
    def __getitem__(self, idx):
        ...
        # 文本处理
        # 核心改动3：读取 {guid}_pro.txt 而非 {guid}.txt
        txt_path = os.path.join(DATA_DIR, f"{guid}_pro.txt")
