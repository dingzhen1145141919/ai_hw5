...
OUTPUT_DIR = "./outputs_optimized"  # 优化后输出目录，区分原有结果
...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Text processing with mild augmentation (优化：适配Twitter英语反讽文本)
        txt_path = os.path.join(DATA_DIR, f"{guid}.txt")
        with open(txt_path, "r", encoding="utf-8", errors="ignore") as f:
            text = f.read().strip()

        if self.split == "train":
            # 优化1：保留emoji（反讽文本核心特征），不做过度裁剪
            if random.random() < 0.1 and len(text.split()) > 5:
                words = text.split()
                keep_idx = random.sample(range(len(words)), int(len(words) * 0.95))  # 保留更多文本
                keep_idx.sort()
                words = [words[i] for i in keep_idx]
                text = " ".join(words)
            # 优化2：针对Twitter缩写/反讽的增强，替换stop words为emoji相关词汇
        ...
        # Image processing with enhanced augmentation (优化：突出情绪特征)
        img_path = os.path.join(DATA_DIR, f"{guid}.jpg")
...
# Model Definition (核心优化：ImageBranch添加视觉注意力层)
...
class ImageBranch(nn.Module):
    ...
    def forward(self, pixel_values):
        # 优化：ViT输出所有patch特征，计算注意力权重
        vit_output = self.vit(pixel_values=pixel_values).last_hidden_state  # (batch_size, seq_len, 768)
        # 计算每个patch的注意力权重，聚焦情绪核心区域
        attn_weights = self.visual_attention(vit_output)  # (batch_size, seq_len, 1)
        # 加权求和得到核心特征
        weighted_feat = torch.sum(vit_output * attn_weights, dim=1)  # (batch_size, 768)
...
# 新增：修改eval_epoch函数，支持bad case保存（适配optimized版本）
                ...
                for idx, (guid, true_id, pred_id) in enumerate(zip(guids, true_labels, preds)):
                    if true_id != pred_id:
                        bad_cases.append({
                            ...
                            "text_weight": float(attn_weights[idx][0].cpu().numpy()) if len(
                                attn_weights) > idx else 0.0,
                            "image_weight": float(attn_weights[idx][1].cpu().numpy()) if len(
                                attn_weights) > idx else 0.0
                        })

    # 保存bad case到CSV（添加optimized标识）
    if save_bad_case and len(bad_cases) > 0:
        bad_case_df = pd.DataFrame(bad_cases)
        bad_case_path = os.path.join(OUTPUT_DIR, f"{case_type}_bad_cases_optimized.csv")
...
def eval_single_modal(model, loader, criterion, modal_type, save_bad_case=False):
    ...
    # 保存单模态bad case（添加optimized标识）
    if save_bad_case and len(bad_cases) > 0:
        bad_case_df = pd.DataFrame(bad_cases)
        bad_case_path = os.path.join(OUTPUT_DIR, f"{modal_type}_bad_cases_optimized.csv")
        bad_case_df.to_csv(bad_case_path, index=False, encoding="utf-8")
...
def train_single_modal(modal_type):
    ...
    patience_counter = 0
    train_log = []
    best_model_path = os.path.join(OUTPUT_DIR, f"best_{modal_type}_model_optimized.pth")
    ...
    for epoch in range(EPOCHS):
        ...
        val_loss, val_metrics = eval_single_modal(
            ...
            save_bad_case=(epoch == EPOCHS - 1 or patience_counter >= PATIENCE - 1)  # 仅在最后一轮/早停前保存
        )
    ...
    # 保存训练日志
    pd.DataFrame(train_log).to_csv(os.path.join(OUTPUT_DIR, f"{modal_type}_train_log_optimized.csv"), index=False)
...
# Multi-modal Fusion Training + 标准早停机制
def train_multi_modal():
    ...
    param_groups = [
        ...
        {"params": model.image_branch.vit.parameters(), "lr": BASE_LR * PRETRAIN_LR_RATIO,
         "weight_decay": WEIGHT_DECAY},
        {"params": model.image_branch.visual_attention.parameters(), "lr": BASE_LR * 1.2, "weight_decay": WEIGHT_DECAY},
        # 注意力层学习率
    ...
    train_log = []
    attn_log = []
    best_model_path = os.path.join(OUTPUT_DIR, "best_multi_modal_model_optimized.pth")
    ...
    # 保存日志
    pd.DataFrame(train_log).to_csv(os.path.join(OUTPUT_DIR, "fusion_train_log_optimized.csv"), index=False)
    pd.DataFrame(attn_log).to_csv(os.path.join(OUTPUT_DIR, "attention_log_optimized.csv"), index=False)
...
def predict_test_set(best_model):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Predicting Test Set"):
            ...
            batch_device = {k: v.to(DEVICE) for k, v in batch.items() if k != "guid"}
            logits, _ = model(
                batch_device["text_input_ids"], batch_device["text_attention_mask"], batch_device["image_pixel_values"]
            )
            preds = torch.argmax(logits, dim=1).cpu().numpy()
    ...
    output_df.to_csv(os.path.join(OUTPUT_DIR, "test_result_optimized.txt"), index=False, header=False)
    print(f"\nTest Set Prediction Completed! Results saved to: {os.path.join(OUTPUT_DIR, 'test_result_optimized.txt')}")
...
def plot_training_curves(fusion_log, text_log, image_log, attn_log):
    ...
    ax1.set_title("Optimized Multi-modal Model Training/Validation Loss Curves", fontsize=14)
    ...
    ax2.set_ylim(0, 1)
    ax2.set_title("Optimized Multi-modal Model Validation Macro-F1 Curve", fontsize=14)
    ...
    ax3.set_title("Text/Image Attention Weight Changes (Optimized)", fontsize=14)
    ...
    ax4.set_ylim(0, 1)
    ax4.set_title("Ablation Study: Optimized Model Macro-F1 Comparison", fontsize=14)
    ax4.grid(True, alpha=0.3)
    ax4.legend(fontsize=10)

    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "training_curves_optimized.png"), dpi=300, bbox_inches="tight")
    print(f"\nTraining curves saved to: {os.path.join(OUTPUT_DIR, 'training_curves_optimized.png')}")
...
if __name__ == "__main__":
    ...
    print("\n===== Final Results Summary (Optimized Model) =====")
    ...
    # 新增：打印bad case文件路径
    print(f"\nBad case files (Optimized Model) saved in {OUTPUT_DIR}:")
    print("- multi_modal_bad_cases_optimized.csv (多模态模型错误样本，含注意力权重)")
    print("- text_bad_cases_optimized.csv (文本单模态错误样本)")
    print("- image_bad_cases_optimized.csv (图像单模态错误样本)")