...
def eval_single_modal(model, loader, criterion, modal_type, save_bad_case=False):
    ...
    if save_bad_case and len(bad_cases) > 0:
        bad_case_df = pd.DataFrame(bad_cases)
        bad_case_path = os.path.join(OUTPUT_DIR, f"{modal_type}_bad_cases.csv")
        bad_case_df.to_csv(bad_case_path, index=False, encoding="utf-8")
...
def train_single_modal(modal_type):
    ...
    patience_counter = 0
    train_log = []
    best_model_path = os.path.join(OUTPUT_DIR, f"best_{modal_type}_model.pth")
    ...
    # 保存训练日志
    pd.DataFrame(train_log).to_csv(os.path.join(OUTPUT_DIR, f"{modal_type}_train_log.csv"), index=False)
...
def train_multi_modal():
    ...
    train_log = []
    attn_log = []
    best_model_path = os.path.join(OUTPUT_DIR, "best_multi_modal_model.pth")
    ...
    # 保存日志
    pd.DataFrame(train_log).to_csv(os.path.join(OUTPUT_DIR, "fusion_train_log.csv"), index=False)
    pd.DataFrame(attn_log).to_csv(os.path.join(OUTPUT_DIR, "attention_log.csv"), index=False)
...
def predict_test_set(best_model):
    ...
    output_df.to_csv(os.path.join(OUTPUT_DIR, "test_result.txt"), index=False, header=False)
    print(f"\nTest Set Prediction Completed! Results saved to: {os.path.join(OUTPUT_DIR, 'test_result.txt')}")
...
def plot_training_curves(fusion_log, text_log, image_log, attn_log):
    ...
    ax1.set_title("Multi-modal Model Training/Validation Loss Curves", fontsize=14)
    ...
    ax2.set_ylim(0, 1)
    ax2.set_title("Multi-modal Model Validation Macro-F1 Curve", fontsize=14)
    ...
    ax3.set_title("Text/Image Attention Weight Changes", fontsize=14)
    ...
    ax4.set_ylim(0, 1)
    ax4.set_title("Ablation Study: Macro-F1 Comparison of Different Modalities", fontsize=14)
...
if __name__ == "__main__":
    ...
    print(f"\nBad case files saved in {OUTPUT_DIR}:")
    print("- multi_modal_bad_cases.csv (多模态模型错误样本)")
    print("- text_bad_cases.csv (文本单模态错误样本)")
    print("- image_bad_cases.csv (图像单模态错误样本)")